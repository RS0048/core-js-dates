...
function getDayName(date) {
  ...
  return arrNameDay[dateNew.getDay()];
}
...
function isDateInPeriod(date, period) {
  ...
  return (
    dateCheck.getTime() >= dateStart.getTime() &&
    dateCheck.getTime() <= dateEnd.getTime()
  );
}
...
function formatDate(date) {
  ...
  if (hours === 12) {
    return `${newDate.getUTCMonth() + 1}/${newDate.getUTCDate()}/${newDate.getUTCFullYear()}, ${hours}:${newDate.getUTCMinutes().toString().padStart(2, '0')}:${newDate.getUTCSeconds().toString().padStart(2, '0')} PM`;
  }
  if (hours > 12) {
    return `${newDate.getUTCMonth() + 1}/${newDate.getUTCDate()}/${newDate.getUTCFullYear()}, ${hours - 12}:${newDate.getUTCMinutes().toString().padStart(2, '0')}:${newDate.getUTCSeconds().toString().padStart(2, '0')} PM`;
  }
  return `${newDate.getUTCMonth() + 1}/${newDate.getUTCDate()}/${newDate.getUTCFullYear()}, ${hours}:${newDate.getUTCMinutes().toString().padStart(2, '0')}:${newDate.getUTCSeconds().toString().padStart(2, '0')} AM`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= countDay; i += 1) {
    date.setDate(i);
    if (date.getDay() === 0 || date.getDay() === 6) countWeekends += 1;
  }
...
function getWeekNumberByDate(date) {
  ...
  do {
    if (
      date.getMonth() === new Date(year, 0, day).getMonth() &&
      date.getDate() === new Date(year, 0, day).getDate()
    )
      return 1;
    day += 1;
  } while (new Date(year, 0, day).getDay() !== 1);
  return (
    Math.floor(
      (date.getTime() - new Date(year, 0, day).getTime()) /
        (7 * 24 * 60 * 60 * 1000)
    ) + 2
  );
}
...
function getNextFridayThe13th(date) {
  ...
  if (day >= 13) month += 1;
  const year = date.getFullYear();
  const dateNew = new Date(year, month, 13);
  while (dateNew.getDay() !== 5) {
    month += 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (dateEnd.getTime() >= dateStart.getTime()) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (dateEnd.getTime() < dateStart.getTime()) break;
      schedule.push(
        `${dateStart.getDate().toString().padStart(2, 0)}-${(dateStart.getMonth() + 1).toString().padStart(2, '0')}-${dateStart.getFullYear()}`
      );
